...
//임시변수를 가져오게 되면 그것에 대한 형변환도 해줘야함!
...
public void orderPick(Scanner scan) {
	...
	if(tmpMenu == null) {
		...
		int tmpPrice = product.get(pick-1).getPrice(); //햄버거가격
		totSum += tmpPrice*count; //누적
		Menu m = new Menu(tmpMenu, count); //메뉴와 개수를 저장
		order.add(m);			
	}
}
//주문리스트
